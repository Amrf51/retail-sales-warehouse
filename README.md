...
The original task wording is preserved in [`coding_challenge.ipynb`](coding_challenge.ipynb). The complete warehouse contract is documented in [`docs/data_model.md`](docs/data_model.md).
...
| 3. Schema and pipeline | Complete | [`sql/schema.sql`](sql/schema.sql), the [`src`](src) pipeline package, and [`tests/test_pipeline.py`](tests/test_pipeline.py) |
...
| 5. Analytical notebook | Complete | Executed [`coding_challenge.ipynb`](coding_challenge.ipynb) with SQL, tables, charts, and interpretation |
...
data/*.csv
    ...
    -> analytical SQL in coding_challenge.ipynb
```
...
├── tests/test_pipeline.py      # Unit and PostgreSQL integration tests
├── coding_challenge.ipynb      # Executed analytical answers
├── compose.yaml
└── Dockerfile
```
...
The PostgreSQL integration test requires an explicitly disposable database because it creates and removes the `retail_sales` schema:
...
The executed challenge notebook is the single source for analytical SQL and detailed results. Its main conclusions are:
...
See [`coding_challenge.ipynb`](coding_challenge.ipynb) for the city tables, tie-aware product analysis, monthly patterns, charts, SQL, and limitations.

## Verification performed

The final project was checked against both the real source files and a disposable PostgreSQL 16 database:
